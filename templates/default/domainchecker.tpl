...
<div class="well">
    ...
    <div class="textcenter" style="margin:5px auto;width:160px;padding:4px;background-color:#fff;border:1px solid #ccc;-moz-border-radius: 6px;-webkit-border-radius: 6px;-o-border-radius: 6px;border-radius: 6px;"><a href="#" onclick="jQuery('#tlds').slideToggle();return false">Search Multiple TLDs</a></div>

    <div class="row hidden" id="tlds">
...
{foreach from=$availabilityresults key=num item=result}
        <tr>
            ...
            <td class="textcenter">{if $result.status eq "unavailable"}<a href="http://{$result.domain}" target="_blank">WWW</a> <a href="#" onclick="popupWindow('whois.php?domain={$result.domain}','whois',650,400);return false">WHOIS</a>{else}<select name="domainsregperiod[{$result.domain}]">{foreach key=period item=regoption from=$result.regoptions}<option value="{$period}">{$period} {$LANG.orderyears} @ {$regoption.register}</option>{/foreach}</select>{/if}</td>
        </tr>
{/foreach}
</table>
