...
<p><input type="button" value="{$LANG.clientareabacklink}" class="btn" onclick="window.location='supporttickets.php'" /> <input type="button" value="Reply" class="btn primary" onclick="jQuery('#replycont2').slideToggle()" /> <input type="button" value="Close Ticket" class="btn error" onclick="window.location='{$smarty.server.PHP_SELF}?tid={$tid}&amp;c={$c}&amp;closeticket=true'" /></p>
